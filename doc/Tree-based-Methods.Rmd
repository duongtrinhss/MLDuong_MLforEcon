...
```{r, fig.align='center'}
knitr::include_graphics("/Users/duongtrinh/Dropbox/FIELDS/Data Science/R_Data Science/R Practice/MLDuong_MLforEcon/figures/Tree.png")
knitr::include_graphics("/Users/duongtrinh/Dropbox/FIELDS/Data Science/R_Data Science/R Practice/MLDuong_MLforEcon/figures/RegressionTree.png")
knitr::include_graphics("/Users/duongtrinh/Dropbox/FIELDS/Data Science/R_Data Science/R Practice/MLDuong_MLforEcon/figures/LinearVsTree.png")
```


# Methods: Bagging, Random Forests, Boosting, and Bayesian Additive Regression Trees
...
We will now discuss `bagging`, `random forests`, `boosting`, and `BART` (Bayesian learners additive regression trees). These are ensemble methods for which the simple building block is a regression or a classification tree. By aggregating many decision trees, the predictive performance of trees can be substantially improved.

Ensemble learning:

$$
f(y \mid \mathbf{x}) = \frac{1}{\mid \mathcal{M}\mid} \sum_{m \in \mathcal{M}} f_m(y \mid \mathbf{x})
$$
where $f_m$ is $m^{th}$ base model.

## Stacking

+ *Stacked generalization*, or *stacking*, as an alternative to using an unweighted average or majority vote, is to learn how to combine the base models by using

$$
f(y \mid \mathbf{x}) = \sum_{m \in \mathcal{M}} w_mf_m(y \mid \mathbf{x})
$$

+ The combination weights used by stacking need to be trained on separate dataset, otherwise they would put all their mass on the best performing base model. 
...
+ In building a random forest, at each split in the tree, the algorithm is *not even allowed to consider* a majority of the available predictors. The main difference between bagging and random forests is the choice
of predictor subset size $m$ taken at each split. Typically, we can choose $m = p, p/2, \sqrt{p},...$ 
...
+ By constrast: Random forests overcome this problem by forcing each split to consider only a subset of the predictors. Therefore, on average $(p − m)/p$ of the splits will not even consider the strong predictor, and so other predictors will have more of a chance. We can think of this process as **decorrelating**
the trees, thereby making the average of the resulting trees less variable and hence more reliable

$\rightarrow$ Using a small value of $m$ in building a random forest will typically be helpful when we have a large number of correlated predictors.
...
1. **Procedure** 
2. $\quad$ (S1) Given a training data set
3. $\quad$ (S2) Select number of trees to build ($n_{trees}$)
4. $\quad$ (S3) **for** $i = 1,2,...,n_{trees}$ **do **
5. $\quad\quad$ (a) Generate a bootstrap sample of the original data.
6. $\quad\quad$ (b) Grow a regression/classification tree to the bootstrapped data
7. $\quad\quad$ **for** each split **do **
8. $\quad\quad\quad$ Select $m_{try}$ variables at random from all $p$ variables
9. $\quad\quad\quad$ Pick the best variable/split-point among the $m_{try}$
10. $\quad\quad\quad$ Split the node into two child nodes
11. $\quad\quad$ **end for**
12. $\quad\quad$ Use typical tree model stopping criteria to determine when a tree is complete (but do not prune)
13. $\quad$ **end for**
14. $\quad$ (S4) Output ensemble of trees.
$$
...
1. **Procedure** 
2. $\quad$ (S1) Set $\hat{f}(x)$ = 0 and $r_i =y_i$ for all $i$ in the training set. 
3. $\quad$ (S2) **for** $b = 1,2,...,B$ **do **
4. $\quad\quad$ (a) Fit a tree $\hat{f}^b$ with $d$ splits ($d + 1$ terminal nodes)  to the training data $(X,r)$.
5. $\quad\quad$ (b) Update $\hat{f}$ by adding in a shrunken version of the new tree:
$$
\hat{f} =  \hat{f}(x) + \lambda \hat{f}^b(x).
$$
6. $\quad\quad$ (b) Update the residuals,
$$
r_i =  r_i - \hat{f}^b(x_i).
$$
7. $\quad$ **end for**
6. $\quad$ (S3) Output the boosted model,
$$
\hat{f}(x) = \sum_{b=1}^B \lambda \hat{f}^{b}(x).
$$
11. **end procedure**
...
rate at which boosting learns. - Typical values are $0.01$ or $0.001$.Very small$\lambda$ can require using a very large value of $B$ in order to achieve good performance.
...
+ $L$ the number if burn-in iterations
+ $\hat{f}_k^b(x)$ the prediction at $x$ for the $k$th regression tree used in the $b$th iteration. At the end of each iteration, the $K$ trees from that iteration will be summed


---
**Algorithm**: Bayesian Additive Regression Trees
--- 

1. **Procedure** 
2. $\quad$ (S1) Let $\hat{f}_1^1(x) = \hat{f}_2^1(x) = ... = \hat{f}_K^1(x) = \frac{1}{nK} \sum_{i=1}^n y_i$
3. $\quad$ (S2) Compute $\hat{f}^1(x) = \sum_{k=1}^K \hat{f}_k^1(x) = \frac{1}{n}\sum_{i=1}^n y_i$
4. $\quad$ (S3) **for** $b = 2,...,B$ **do **
5. $\quad\quad$ (a) For $k = 1,2,...,K:$
6. $\quad\quad\quad$ (i) For $i = 1,2,...,n$, compute the current partial residual:
$$
r_i = y_i - \sum_{k'<k} \hat{f}_{k'<k}^b(x_i) - \sum_{k' > k} \hat{f}_{k'}^{b-1} (x_i)
$$
7. $\quad\quad\quad$ (ii) Fit a new tree, $\hat{f}_{k}^b(x)$, to $r_i$, by randomly perturbing the $k$th tree from the previous iteration, $f_k^{b-1}(x)$. Pertubations that improve the fit are favored.
8. $\quad\quad$ (b) Compute $\hat{f}^{b}(x) = \sum_{k = 1}^K \hat{f}_k^b(x)$.
9. $\quad$ **endfor**
10. $\quad$ Compute the mean after $L$ burn-in samples,
$$
...
A key element of the BART approach is that in Step 3- (a) - (ii), rather than fitting a fresh tree to the current partial residual, BART randomly chooses a perturbation to the tree from the previous iteration ($\hat{f}^{b-1}_k$) from a set of possible perturbations, favoring ones that improve the fit to the partial residual. There are two components to this perturbation:
...
```{r, results='hide'}
str(Boston) # 506 obs. of  13 variables
```
...
ggplot(data_frame(yhat, boston.test), aes(x = yhat, y = boston.test)) +
  geom_point() +
  coord_cartesian(xlim = c(0,50), ylim = c(0,50)) +
  geom_abline(intercept = 0, slope = 1) +
    ggtitle(paste0("MSE = ", round(mean((yhat - boston.test)^2),3))) +
...
# abline(0, 1)
ggplot(data_frame(yhat.bag, boston.test), aes(x = yhat.bag, y = boston.test)) +
  geom_point() +
  coord_cartesian(xlim = c(0,50), ylim = c(0,50)) +
  geom_abline(intercept = 0, slope = 1) +
  ggtitle(paste0("MSE = ", round(mean((yhat.bag - boston.test)^2),3))) +
    theme(plot.title = element_text(hjust = 0.5))
```
...
ggplot(data_frame(yhat.rf, boston.test), aes(x = yhat.rf, y = boston.test)) +
  geom_point() +
  coord_cartesian(xlim = c(0,50), ylim = c(0,50)) +
  geom_abline(intercept = 0, slope = 1) +
  ggtitle(paste0("MSE = ", round(mean((yhat.rf - boston.test)^2),3))) +
    theme(plot.title = element_text(hjust = 0.5))
```
...
+ The second is a measure of the total decrease in node impurity that results from splits over that variable, averaged over all trees (this was plotted in Figure 8.9). In the case of regression trees, the node impurity is measured by the training RSS, and for classification trees by the deviance. Plots of these importance measures can be produced using the `varImpPlot()` function.
...
```{r, fig.cap="Pruning complexity parameter (cp) plot illustrating the relative cross validation error (y-axis) for various cp values (lower x-axis). Smaller cp values lead to larger trees (upper x-axis). Using the 1-SE rule, a tree size of 10-12 provides optimal cross validation results.", fig.align='center'}
plotcp(ames_dt1)
```

```{r, fig.cap="Pruning complexity parameter plot for a fully grown tree. Significant reduction in the cross validation error is achieved with tree sizes 6-20 and then the cross validation error levels off with minimal or no additional improvements.", fig.align='center'}
ames_dt2 <- rpart(
    formula = Sale_Price ~ .,
    data    = ames_train,
    method  = "anova", 
...
```{r, fig.cap="Cross-validated accuracy rate for the 20 different $alpha$ parameter values in our grid search. Lower $alpha$ values (deeper trees) help to minimize errors."}
...
# Simulation Study
```{r}
heart <- read.csv("/Users/duongtrinh/Dropbox/FIELDS/Data Science/R_Data Science/R Practice/MLDuong_MLforEcon/data/Heart.csv")

str(heart) # 03 obs. of  15 variables

hitters <- read.csv("/Users/duongtrinh/Dropbox/FIELDS/Data Science/R_Data Science/R Practice/MLDuong_MLforEcon/data/Hitters.csv")
